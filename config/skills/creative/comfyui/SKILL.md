...
12. **`pipx` bad interpreter issue** — on macOS the bundled `pipx` may point to a
    missing Python interpreter (e.g., after a Homebrew upgrade). Resolve by
    reinstalling `pipx` via Homebrew (`brew reinstall pipx`) or using the
    fallback `pip install --user comfy-cli` as demonstrated earlier.

13. **`comfyui_setup.sh` unbound variable bug** — the script expects the
    `WORKSPACE_ARG` array to be defined. If you encounter the error
    `line XX: WORKSPACE_ARG[@]: unbound variable`, run the script with the
    environment variable `WORKSPACE_ARG=()` beforehand, or edit the script to
    initialize it (`WORKSPACE_ARG=()`). This ensures the hardware‑check path
    proceeds correctly.

14. **Marginal hardware verdict handling** — when the hardware check returns
    `"verdict": "marginal"` (e.g., Apple M1 Pro with 16 GB RAM), the skill now
    explicitly warns that SDXL or Flux may be slow or OOM and recommends the
    user consider Comfy Cloud for heavier models before proceeding.

15. **`comfy` command not found after `pipx` install** — if `comfy` is missing
    from `$PATH` after a successful `pipx install comfy-cli`, ensure the pipx
    binary directory is exported (`export PATH="$HOME/.local/bin:$PATH"`).
    Alternatively, invoke via `python3 -m comfy_cli` or use the `uvx` wrapper
    (`uvx --from comfy-cli comfy ...`).

These additions capture the real-world debugging steps observed in this session.
